...
# crontab -e
# 00 20 * * * Rscript /home/ubuntu/srv/my-statistical-health/imputeme/space_check_cron_job.R > /home/ubuntu/misc_files/cron_logs/`date +\%Y\%m\%d\%H\%M\%S`-space-check-cron.log 2>&1
...
if(percentage > 90){
  print("should send warning mail")  
  	...
  	mailingResult<-try(send.mail(from = email_address,
  															 ...
  															 subject = "Statistical-health is almost full",
  															 body = paste0("The data-partition of the hub server is at ",percentage,"% capacity"),
  															 html=T,
  															 smtp = list(
